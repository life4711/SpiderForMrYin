...
#爬虫类
class Spider:
    def __init__(self,parll,headerss,id,num):
        self.id = id
        self.num = num+1
        self.par1 = parll
    ...
    def wExcel(self, outEfile):
        ...
        w.save(outEfile)
        print u'爬虫报告：会刊目录已保存至当前文件夹下"huikan+当前时间.xls"文件'
...
----------------------------------------------------------
   ...
   日期：2016-05-06
   语言：Python 2.7
   功能：将会刊网中指定会刊的参展人信息保存到当前目录下的"huikan+当前时间.xls"文件
...
data = {"Username":"handwudi","Passwrod":"x2008bzz","B1":"登陆"}
print u'请输入会刊ID:'
id = raw_input()
print u'请输入爬虫需要爬取的页数:'
num = raw_input()
myspider = Spider(data,headers,int(id),int(num))
myspider.Solve()